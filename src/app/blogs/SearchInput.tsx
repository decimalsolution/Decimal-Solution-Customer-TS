...
const SearchInput = ({ newblogs }: SearchProps) => {
  ...
  const filteredBlogs = sortedBlogs.filter((item) => item.blogTitle.toLowerCase().includes(searchKey.toLowerCase()));
    ...
    <div>
      ...
      <div className="mt-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-items-center gap-8 px-16">
        {currentBlogs.length > 0 ? (
          currentBlogs.map((blog, index) => <BlogView key={"blog-" + index} blog={blog} />)
        ) : (
          <p>No blogs found</p>
        )}
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-10 px-4">
          <div className="w-full overflow-x-auto">
            <Pagination className="flex justify-center items-center space-x-2 md:space-x-4">
              <PaginationContent className="flex flex-wrap items-center justify-center gap-2">
                {currentPage > 1 && (
                  <PaginationItem>
                    <PaginationPrevious
                      onClick={() => handlePageChange(currentPage - 1)}
                      className="cursor-pointer px-2 py-1 text-sm md:text-base"
                    />
                  </PaginationItem>
                )}
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <PaginationItem key={page}>
                    {page === currentPage ? (
                      <PaginationLink isActive className="px-3 py-1 text-xs md:text-sm">
                        {page}
                      </PaginationLink>
                    ) : (
                      <PaginationLink

...
const DropZone: React.FC<DropZoneProps> = ({ onChange, getValues, name, setValue, className, error }) => {
      ...
      >
        {getValues(name) == null ? (
          ...
          <div className="relative w-full h-full">
            <iframe src={getValues(name).preview || getValues(name)} className="w-full h-full object-cover" />
            <button
        ...
        {error?.[name] && <p className="text-red-500 text-sm mt-2">{error?.[name]?.message?.toString()}</p>}
      </div>
    </div>
  );
};
